...
def read_sourceterms(s):
    ...
    for i, row in enumerate(u.read_rows(fname, header=["term","elems"], delimiter="|")):
        term = {"term":row["term"],"source_locs":[]***REMOVED***
        for elem in row["elems"].split(":"):
            try:
                ...
                continue
            term["source_locs"].append({"source_id":source_id,"lan":lan***REMOVED***)
        yield term
...
def read_PE():
    for r in u.read_rows(PE_file, delimiter=";"):
        yield {"source_locs":[{"source_id":r["UI"],"lan":"nl"***REMOVED***],"term":r["PRINT_ENTRY_NL"]***REMOVED***

def read_MSH():
    for r in u.read_rows(MSH_file, delimiter=";"):
        yield {"source_locs":[{"source_id":r["UI"],"lan":"nl"***REMOVED***],"term":r["MH_NL"]***REMOVED***
...
def build_input_que(terms, source, input_que):
    ...
    for term in terms:
        ...
        batch.append(term)
        if len(batch)==250:
            input_que.put({"batch":batch, "source":source***REMOVED***)
            batch = []
...
def check_term(t, source_locs, source):
    if u.check_row({"STR":t***REMOVED***,umls=False):
        t = u.normalize(t)
        ...
        for elem in source_locs:
            elem["source"]=source
        return {"term":t, "source_locs":source_locs***REMOVED***
