...
def add():
    ...
    result = { "msg": "Added %s with %s concepts" % (body["parent"]["name"], len(body["concepts"]))}
...
def extract():
    ...
    try:
        ...
        result["title"] = soup.find("h1", { "id" : "firstHeading" }).text.strip()
...
def remove():
    ...
    try:
        ...
        db = Graph()
        db.cypher.execute("MATCH (g:Group {name: {N} }) OPTIONAL MATCH (n)<-[r]-(c) DELETE r,n,c", {"N": body["group_name"]})
